...
/**
 * 'user_signed_up' (welcome email) is the only implemented event kind —
 * see PLAN.md "Welcome email on signup". Everything else (gig_funded,
  ...
  private async sendWelcomeEmail(email: string, name: string): Promise<void> {
    ...
    const response = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        from,
        to: email,
        subject: 'Welcome to Sorted — let’s get things sorted',
        html: this.welcomeEmailHtml(firstName),
      }),
    });
